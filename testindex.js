...
function init() {
    ...
    boss2Img.name = 'boss2';
    boss2Img.onload = preloader;

      // Boss
    boss3Img.src = 'img/boss.png';
    boss3Img.name = 'boss3';
    boss3Img.onload = preloader;
...
function update() {
    ...
    if (PopInvaTime < 0) {
        if (score < 400) {
            ...
            console.log("sup");
            PopInvaTime = Math.floor(Math.random() * 10 + 40);
        }
        addEnemy();
    } else{
        PopInvaTime--;
    ...
    for(var i = 0; i < bullets.children.length; i++) {
        ...
        // decrementation de la vie du boss, ajout des points et suppression des balles si elle le touche
        if (boss2 && bullets.children[i].x >= boss2.x && bullets.children[i].x + 4 < boss2.x + 256 && bullets.children[i].y < boss2.y + 200) {
            TextForScore.text = parseInt(TextForScore.text + 50);
            score += 70;
            bullets.removeChildAt(i);
            boss2Live--;
        ...
        // decrementation de la vie du boss, ajout des points et suppression des balles si elle le touche
        if (boss3 && bullets.children[i].x >= boss3.x && bullets.children[i].x + 4 < boss3.x + 256 && bullets.children[i].y < boss3.y + 200) {
            TextForScore.text = parseInt(TextForScore.text + 50);
            score += 100;
            bullets.removeChildAt(i);
            boss3Live--;
    ...
    // ajout du boss
    if (score >= 500 && boss == null && boss1 == false) {
        console.log("boss1");
        shootBoss = true;
    ...
    // ajout du boss
    if (score >= 4000 && boss2 == false){
        console.log("boss2");
        shootBoss2 = true;
        boss2 = new createjs.Bitmap(boss2Img);
        boss2.x = 160;
        boss2.y = 15;
...
console.log(boss2.x);
    ...
    // ajout du boss
    if (score >= 9000 && boss3 == null) {
console.log("boss3");
        shootBoss3 = true;
        boss3 = new createjs.Bitmap(boss3Img);
        boss3.x = 160;
        boss3.y = 15;
    ...
    if (boss2) {
        ...
        if(boss2.x < 0) {
            ...
            console.log("mouve boss 2");
            boss2Moves *= -1;
        }

        boss2.x += (10 * boss2Moves);
        // tir du boss tout les 700
        if (d - lastFireInva > 700) {
            shoot(boss2, boss2.x, boss2.y);
            lastFireInva = d;
        }
    }
    ...
    if (boss3) {
        ...
        boss3.x += (10 * boss3Moves);
        // tir du boss tout les 700
        if (d - lastFireInva > 700) {
            shoot(boss3, boss3.x, boss3.y);
            lastFireInva = d;
        }
    }
    ...
    if(boss3Live <= 0) {
        invaPop = true;
        shootBoss2 = false;
        stage.removeChild(boss3);
        win = new createjs.Bitmap(winImg);
        stage.addChild(win);
        stage.removeChild(invaders, ship, boss, boss2, boss3);
        loseFunc();
    }
